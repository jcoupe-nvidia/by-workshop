...
Owns:
    - JSON syntax repair (trailing commas, single quotes, missing braces, etc.)
    - Extraction of JSON objects from mixed text/JSON output
    - Structural repair (flat tool calls, missing optional fields, etc.)
    - Fuzzy tool-name matching
    - Unsafe argument detection
    - Repair-vs-reject policy

Does NOT own:
    - Structured event emission (see runtime.tracing)
    - Tool implementations or registry (see runtime.tools)
    - Schema validation of well-formed output (see runtime.schemas)
    - Environment state or transitions
...
def parse_with_fallback(
    ...
    if result.action == FallbackAction.NO_ACTION:
        ...
        from src.runtime.schemas import extract_json
