...
- Async GRPO readiness with explicit trajectory freshness metadata
...
This repo is forward-compatible with asynchronous GRPO rollout collection. In standard (synchronous) GRPO, rollouts are collected with the current policy weights and used for a training step immediately. In async GRPO, rollout workers generate trajectories concurrently while the policy is being updated, so a trajectory may have been generated by a slightly older version of the model than the one currently being trained.

This creates a risk: stale trajectories can silently degrade training if mixed in without accounting for the weight mismatch. The repo addresses this by embedding four metadata fields into every trajectory and DatumSpec:

| Field | Purpose |
| --- | --- |
| `gen_weight_version` | Policy weight version that generated this trajectory |
| `train_weight_version` | Policy weight version the trajectory is intended for |
| `trajectory_age_ms` | Time elapsed since generation, in milliseconds |
| `replay_status` | Whether the trajectory was accepted, rejected as stale, or replayed with correction |

During synchronous runs (the default), these fields carry placeholder values (`gen_weight_version=0`, `trajectory_age_ms=0`, `replay_status="accepted"`). When async collection is enabled, the rollout infrastructure populates them with real values. The `is_fresh(episode, max_age_ms)` helper in `training/nemo_rl_adapter.py` enforces staleness limits — it returns `True` for synchronous runs and filters trajectories that exceed the age threshold under async collection.

The constants are defined in `src/rollouts/trace_types.py`, propagated through `episode_to_datum_spec()` in `src/training/nemo_rl_adapter.py`, and populated in the training dataset in `src/training/run_grpo_training.py`. No changes to `envs/` or `eval/` are needed — async metadata is a training-layer concern only.
